...
for i, file in enumerate(files):
    ...
    grouped_df = df.groupby(['Ls', 'latitude']).agg('cdod').mean().reset_index()
    ...
    sc = ax.scatter(grouped_df['Ls'], grouped_df['latitude'], c=grouped_df['cdod'], cmap=cmap, vmin=vmin, vmax=vmax)
    ax.set_xlabel('Ls', fontsize=16)
...
cax = fig.add_axes([0.9, 0.2, 0.02, 0.6])  # Position of the colorbar
cbar = fig.colorbar(sc, cax=cax, orientation='vertical')
cbar.ax.tick_params(labelsize=16)
fig.suptitle('Kondisi Column Dust Optical Depth pada MY24 - 35\n', weight='bold', ha='center', fontsize=18)
fig.subplots_adjust(right=0.88, wspace=0.27, top=0.95, hspace=0.3)  # Adjust spacing between subplots
plt.show()
