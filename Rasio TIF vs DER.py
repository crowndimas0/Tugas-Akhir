...
# Read the CSV file
df = pd.read_csv('/content/TER surface and TER space - Sheet2.csv')
...
for i, MY in enumerate(MY_numbers):
    ...
    if MY in ['MY25', 'MY28', 'MY34']:
        ...
        # Add title for MY25, MY28, and MY34
        ax.set_title('{}\n(Tahun Badai Debu Global)'.format(MY))
    else:
        ax.scatter(y=filtered_df[MY_column], x=np.log10(filtered_df[Reff_column]), alpha=0.5)

    # Set labels
    ax.set_xlabel('$Dust$ $Effective$ $Radius$', fontsize = 16)
    ax.set_ylabel('Rasio $Thermal$ $Infrared$ $Flux$', fontsize = 16)

# Adjust subplot spacing and display plot
fig.suptitle('Hubungan Rasio Thermal Infrared Flux dan Dust Effective Radius\n', weight='bold', fontsize=16)
fig.tight_layout()
plt.show()
